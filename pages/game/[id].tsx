...
const Game: React.FC<GameId> = ({ game }) => {
	...
	const buttons = [
		<button className={styles.buy} key='buy'>
			{game.price === 'Бесплатно' ? 'Получить' : 'Купить сейчас'}
		</button>,
		<button className={styles.add} key='add'>
			Добавить в корзину
		</button>,
		<button className={styles.toList} key='toList'>
			В список желаемого
		</button>
	]
